...
import {
  LaborPaymentVoucherServices,
  ILaborPaymentVoucher,
  ApiBaseState,
  ErrorModel,
  QueryParams,
  ResultModel,
...
export function useCreateLaborPaymentVoucher(): UseMutationResult<ResultModel, ErrorModel, FormData, unknown> {
...
export function useUpdateLaborPaymentVoucherById(
  id: string
...
export function useDeleteLaborPaymentVoucherById(
  id: string
...
export function useDeleteMultipleLaborPaymentVouchers(): UseMutationResult<
  ResultModel,
  ErrorModel,
  Array<string>,
  unknown
> {
  return useMutation<ResultModel, ErrorModel, Array<string>, unknown>({
    ...
    mutationFn: (data: Array<string>) =>
      LaborPaymentVoucherServices.getInstance().deleteMultipleLaborPaymentVouchers(data),
  });
}
...
export function useGetLaborPaymentVoucherById(
  id: string
): UseQueryResult<ApiBaseState<ILaborPaymentVoucher>, ErrorModel> {
...
export function useGetLaborPaymentVoucherForViewById(
  id: string
): UseQueryResult<ApiBaseState<ILaborPaymentVoucher>, ErrorModel> {
  const enabled = id.length > 0 ? true : false;
  return useQuery<ApiBaseState<ILaborPaymentVoucher>, ErrorModel>({
...
export function useGetLaborPaymentVoucherForUpdateById(
  id: string
): UseQueryResult<ApiBaseState<ILaborPaymentVoucher>, ErrorModel> {
  const enabled = id.length > 0 ? true : false;
  return useQuery<ApiBaseState<ILaborPaymentVoucher>, ErrorModel>({
