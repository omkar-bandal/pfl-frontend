...
export const TransportPaymentVoucherTable = () => {
  ...
  const tpVoucherColumns = useTPVoucherColumns(canEdit, canView, navigate);

  const tableNavActionConfig: TableNavActionsConfig = {
    tableId: dataTableIds.TP_VOUCHER_TABLE_ID,
    createPath: operationsRoutes.CREATE_TRANSPORT_CASH_VOUCHER,
    editPath: operationsRoutes.UPDATE_TRANSPORT_CASH_VOUCHER,
    viewPath: operationsRoutes.VIEW_A_TRANSPORT_CASH_VOUCHER,
  };
    ...
    if (selectedRows.length === 0) {
      ...
      mutateAsync(selectedRows as Array<string>)
        .then(() => {
          ...
          setTimeout(() => {
            navigate(operationsRoutes.VIEW_ALL_TRANSPORT_CASH_VOUCHER);
          }, 2000);
        })
        .catch(() => {
