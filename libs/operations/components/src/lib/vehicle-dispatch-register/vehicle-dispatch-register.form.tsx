...
import { FormButtonGroup, PageTitle, RadioGroupInput, SelectInput, TextInput, toast } from '@prime-fresh/shared/components';
import {
  operationsRoutes,
...
export const VehicleDispatchRegisterForm = () => {
  ...
  const {
    ...
    error: PatchError,
    data: PatchData,
  } = useUpdateVehicleDispatchRegister(vehicleDispatchId);
    ...
    vehicleDispatchId === ''
      ? mutateAsyncPost(values)
          .then(() => {
            ...
            setTimeout(() => {
              navigate(operationsRoutes.VIEW_ALL_VEHILCE_DISPATCH_REGISTER);
            }, 2000);
          })
          .catch(() => {
  ...
  return vehicleDispatchId !== '' && isLoading ? (
    <Box sx={{ flex: 1 }}>
      <LinearProgress />
    </Box>
  ) : (
    <>
      <Formik
        ...
        validationSchema={vehicleDispatchRegisterSchema}
        validateOnBlur={true}
              ...
              >
                <FormButtonGroup
                  ...
                  onPreview={() => {
                    ...
                    dispatch(setVehicleDispatchRegisterFormPreview(values));
                  }}
                />
              </Grid2>
            </Grid2>
      ...
      <VehicleDispatchRegisterFormPreview />
    </>
  );
};
