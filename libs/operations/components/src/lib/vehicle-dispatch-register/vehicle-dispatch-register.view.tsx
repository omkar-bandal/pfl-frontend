...
import { useGetVehicleDispatchRegisterForViewById } from '@prime-fresh/operations/modules';
...
export const VehicleDispatchRegisterView = () => {
  ...
  const {
    ...
    isPending,
    isError,
  } = useUpdateDocStatusWithOneApproval(vehicleDispatchId);
  const changeVehicleDispatchStatus = (status: 'approved' | 'reject') => {
    mutateAsync({
      ...
      .catch(() =>
        toast.error(error?.message ? error?.message : 'Error while changing status of vehicle dispatch register.')
      );
  };
  return isLoading ? (
    <Box flex={1}>
      ...
      <Grid container rowSpacing={1}>
        ...
        <Grid item xs={12}>
          <DrawerContainer>
            ...
            >{`Current Status: ${convertInTitleCase(vehicleDispatchData?.overAllStatus || '')}`}</Typography>
          ...
          <Typography variant="subtitle1" component="span" sx={{ fontWeight: 700, color: '#2C3E50' }}>
            ...
            <Typography
              ...
              sx={{
                marginLeft: 2,
                fontWeight: 700,
                color: getDocStatusColor(vehicleDispatchData?.overAllStatus || 'hold'),
              }}
            >
