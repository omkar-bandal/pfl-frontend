...
import { IGRN, queryClient } from '@prime-fresh/services';
...
export const GRNForm = () => {
  ...
  const {data: emp, isLoading: isEmpLoading} = useGetEmployeePartialData();
  const employees = useMemo(() => emp?.data ? mapToValueLabelArray(emp?.data, 'id', 'fullName') : [], [emp]);
  ...
  const grnValidationSchema = useMemo(() => grnId === '' ? grnSchemaForCreate : grnSchemaForUpdate, [grnId]);
  
  const formik = useFormik({
    enableReinitialize: true,
    initialValues: initialValuesForGRN,
    validationSchema: grnValidationSchema,
    validateOnChange: true,
    validateOnBlur: true,
    onSubmit: (values) => handleSubmit(values),
  });
  const { mutateAsync: mutatePost, error: errorPost, data: resPost } = useCreateGRN();
  const { mutateAsync: mutatePatch, error: errorPatch, data: resPatch } = useUpdateGRN(grn?.data?.id || '');
  ...
  const handleSubmit = (values: Omit<IGRN, 'id'>) => {
    ...
    (mutatePost(formData)
      .then(() => {
        toast.success(resPost ? resPost.message : 'GRN created.');
      ...
      .catch(() => {
        toast.error(errorPost ? errorPost.message : 'Error while creating GRN.');
      })) :
    (mutatePatch(formData)
      .then(() => {
        toast.success(resPatch ? resPatch.message : 'GRN updated.');
        queryClient.invalidateQueries({ queryKey: ['user-notifications'] });
        setTimeout(() => {
          navigate(operationsRoutes.VIEW_ALL_GRN);
        }, 2000);
      ...
      .catch(() => {
        toast.error(errorPatch ? errorPatch.message : 'Error while updating GRN.');
      }))
  };
          ...
          <Grid2 container columnSpacing={1} rowSpacing={1} padding={1}>
            ...
            <Grid2 size={{ xs: 12, md: 7 }}>
              <RadioGroupInput
                ...
                name="purchaseType"
                alignment="vertical"
                options={staticData.PURCHASE_TYPE}
            ...
            <Grid2 size={{ xs: 12, md: 4 }}>
              <AutoCompleteInput
                ...
                handleChange={(event, newValue) => {
                  if (newValue !== null) {
                    if (typeof newValue === 'string') formik.setFieldValue(`purchaseInstructionsBy`, null);
                    else {
                      formik.setFieldValue(`purchaseInstructionsBy`, newValue.value);
                    }
                  } else formik.setFieldValue('purchaseInstructionsBy', null);
                }}
              />
            </Grid2>
            <Grid2 size={{ xs: 12, md: 4 }}>
                      ...
                      >
                        ...
                        <Grid2 size={{ xs: 6, md: 3 }}>
                          <TextInput
                            ...
                            handleChange={(event) =>
                              handleGRNProductsChange(index, 'quantity', Number(event.target.value), formik)
                            }
                          />
                        </Grid2>
                        <Grid2 size={{ xs: 6, md: 3 }}>
                          <TextInput
                            ...
                            handleChange={(event) =>
                              handleGRNProductsChange(index, 'unitPrice', Number(event.target.value), formik)
                            }
                          />
                        </Grid2>
                        <Grid2 size={{ xs: 6, md: 3 }}>
                        ...
                        <Grid2 size={{ xs: 6, md: 4 }}>
                          <TextInput
                            ...
                            handleChange={(event) =>
                              handleGRNProductsChange(index, 'grossWeight', Number(event.target.value), formik)
                            }
                          />
                        </Grid2>
                        <Grid2 size={{ xs: 6, md: 4 }}>
            ...
            <Grid2 size={{ xs: 12, md: 3 }}>
              <TextInput
                ...
                handleChange={(e) => handleGRNProductsChange(null, 'freight', Number(e.target.value), formik)}
              />
            </Grid2>
            <Grid2 size={{ xs: 12, md: 3 }}>
              <TextInput
                ...
                handleChange={(e) => handleGRNProductsChange(null, 'otherCharges', Number(e.target.value), formik)}
              />
            </Grid2>
            <Grid2 size={{ xs: 12, md: 3 }}>
              <TextInput
            ...
            >
              <FormButtonGroup
                ...
                isSubmitError={errorPatch || errorPost}
                resetLabel="Reset"
