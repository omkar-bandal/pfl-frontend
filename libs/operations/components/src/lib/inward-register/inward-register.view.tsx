...
import { useGetInwardRegisterForViewById } from '@prime-fresh/operations/modules';
import {
  BtnSmall,
  DrawerContainer,
  InfoTooltip,
...
import {
  ...
  useUpdateDocStatusWithOneApproval,
...
export const InwardRegisterView = () => {
  ...
  const changeInwardRegisterStatus = (status: 'approved' | 'reject') => {
    mutateAsync({
      ...
      .catch(() =>
        toast.error(error?.message ? error?.message : 'Error while changing status of arrival quality report.')
      );
  };
  return (
          ...
          <Grid container rowSpacing={1} marginBottom={2}>
            ...
            <Grid item xs={12}>
              <DrawerContainer>
                ...
                >{`Current Status: ${convertInTitleCase(inwardData?.overAllStatus || '')}`}</Typography>
            ...
            <Grid item xs={12} alignItems="center">
              <Typography variant="subtitle1" component="span" sx={{ fontWeight: 700, color: '#2C3E50' }}>
                ...
                <Typography
                  variant="subtitle1"
                  component="span"
                  sx={{ marginLeft: 2, fontWeight: 700, color: getDocStatusColor(inwardData?.overAllStatus || 'hold') }}
                >
                ...
                <div className={styles.px}>
                  <table className={styles.voucherTable}>
                    ...
                    <tbody>
                      ...
                      {(inwardData?.inwardProducts.length || 0) < 5 &&
                        Array(5 - (inwardData?.inwardProducts?.length || 0))
                          .fill(null)
                          .map(() => (
