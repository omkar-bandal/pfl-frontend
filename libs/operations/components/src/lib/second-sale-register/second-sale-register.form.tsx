...
import {ISecondSaleRegister} from '@prime-fresh/services';
...
import { handlePushProduct, handleRemoveProduct, handleSecondSaleProductsChange } from './helper-function';
...
export const SecondSaleRegisterForm = () => {
    ...
    secondSaleId === ''
      ? mutateAsyncPost(values)
          .then(() => {
            ...
            setTimeout(() => {
              navigate(operationsRoutes.VIEW_ALL_SECOND_SALE_REGISTER);
            }, 2000);
          })
          .catch(() => {
  ...
  return secondSaleId !== '' && isLoading ? (
    ...
    <>
      <FormikProvider key={secondSaleId === '' ? 'create-ss-register' : 'update-ss-register'} value={formik}>
        <form
          key={secondSaleId === '' ? 'create-form' : 'update-form'}
                  ...
                  <>
                    ...
                    <Grid2 size={{ xs: 12 }} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'end' }}>
                      <AddFieldButton
                        label="Add More"
                        onClickFn={() =>
                          handlePushProduct(SecondSaleProductsInitialValue, formik.values, formik.setFieldValue)
                        }
                      />
                    </Grid2>
                  </>
