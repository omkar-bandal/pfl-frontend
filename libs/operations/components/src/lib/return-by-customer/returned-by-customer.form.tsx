...
import {
  calculateRejectedProductDetails,
  calculateReturnedProductDetails,
  handleReturnedProductsChange,
...
export const ReturnedByCustomerForm = () => {
  ...
  const rbcCompanyData = React.useMemo(
    () => (company?.data ? company?.data.find((data) => data.id === dcData?.companyName) : null),
    [dcData]
  );
  console.log('RBC Company Data', rbcCompanyData);

  const { data: loc } = useGetBranchesPartialData();
  const rbcLocationData = React.useMemo(
    () => (loc?.data ? loc?.data.find((data) => data.id === dcData?.fromLocation) : null),
    [dcData]
  );
  console.log('RBC Location Data', rbcLocationData);

  const { data: custNames } = useGetCustomerNames();
  const rbcCustomer = React.useMemo(
    () => (custNames?.data ? custNames?.data?.find((data) => data.id === dcData?.customerName) : null),
    [dcData]
  );
    ...
    rbcId === ''
      ? mutateAsyncPost(values)
          .then(() => {
            ...
            setTimeout(() => {
              navigate(operationsRoutes.VIEW_ALL_RBC);
            }, 2000);
          })
          .catch(() => {
                      ...
                      >
                        ...
                        <Grid2 size={{ xs: 12, md: 2.4 }}>
                          <TextInput
                            ...
                            handleChange={(e) => calculateReturnedProductDetails(e, index, formik)}
                            slotProps={{
                              input: {
                        ...
                        <Grid2 size={{ xs: 12, md: 2.4 }}>
                          <TextInput
                            ...
                            handleChange={(e) => calculateRejectedProductDetails(e, index, formik)}
                          />
                        </Grid2>
                        {/* <Grid2 size={{ xs: 12, md: 3 }}>
                          <TextInput
