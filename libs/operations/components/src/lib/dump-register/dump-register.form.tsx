...
export const DumpRegisterForm = () => {
  ...
  const { data: dumpRecord, isLoading, isError, error } = useGetDumpRegisterForUpdateById(dumpRegiId);
  ...
  const calculateTotalDumpCost = useCallback(
    (fieldName: 'quantity' | 'unitPrice', newValue: number, index: number, formik: any) => {
      if (index !== null) {
        ...
        formik.setFieldValue('totalCostInWords', numToWords(Number(totalDumpCost)));
      }
    },
    [recalcTotalDumpCost]
  );
  const handleRemoveDumpProduct = useCallback(
    (index: number, formik: any): void => {
      const updatedProducts = formik.values.dumpProducts.filter((_: any, i: number) => i !== index);
      const totalDumpCost = recalcTotalDumpCost(updatedProducts).toFixed(2);
      formik.setFieldValue('dumpProducts', updatedProducts, false);
      formik.setFieldValue('totalDumpCost', Number(totalDumpCost));
      formik.setFieldValue('totalCostInWords', numToWords(Number(totalDumpCost)));
  ...
  const handlePreview = useCallback(() => {
    const previewData = {
      ...formik.values,
      companyName: companies?.find((company) => company.value === formik.values.companyName)?.label || null,
      location: allLocations.find((loc) => loc.value === formik.values.location)?.label || null,
      grn: grnNums.find((grn) => grn.value === formik.values.grn)?.label || null,
    ...
    dumpRegiId === ''
      ? mutateAsyncPost(values)
          .then(() => {
            ...
            setTimeout(() => {
              navigate(operationsRoutes.VIEW_ALL_DUMP_REGISTERS);
            }, 2000);
          })
          .catch(() => {
  ...
  return dumpRegiId !== '' && isLoading ? (
    ...
    <>
      <FormikProvider key={dumpRegiId === '' ? 'create-dump' : 'update-dump'} value={formik}>
        <form
              ...
              <FieldArray name="dumpProducts">
                {({ push }) => (
                  <>
                      ...
                      >
                        ...
                        >
                          ...
                          {formik.values.dumpProducts.length > 1 && (
                            <IconButton
                              color="error"
                              size="small"
                              onClick={() => handleRemoveDumpProduct(index, formik)}
                            >
                              <Close fontSize="small" />
                            </IconButton>
                          )}
                        </Grid2>
                        <ProductFormFields fieldArrayName="dumpProducts" index={index} formik={formik} />
                        ...
                        <Grid2 size={{ xs: 12, md: 3 }}>
                          <TextInput
                            ...
                            handleChange={(e) =>
                              calculateTotalDumpCost('quantity', Number(e.target.value), index, formik)
                            }
                          />
                        </Grid2>
                        <Grid2 size={{ xs: 12, md: 3 }}>
                          <TextInput
                            ...
                            handleChange={(e) =>
                              calculateTotalDumpCost('unitPrice', Number(e.target.value), index, formik)
                            }
                            slotProps={{
                              input: {
                    ...
                    >
                      <AddFieldButton label="Add More" onClickFn={() => push(dumpProductsInitialValue)} />
                    </Grid2>
                  </>
                )}
              </FieldArray>
