...
import { PURCHASE_ARRAYS, PURCHASE_ROUTES, multicashVoucherSchema, useCreateMultiCashVoucher, useGetMultiCashVoucherForUpdateById, useUpdateMultiCashVoucherById, multipleCashVoucherInitialValue, particularsInitialValue, setMCVoucherFormPreview } from "@prime-fresh/purchase/modules";
...
import { useGetCompanyNames, useGetAllGRNNums, useGetAllDeliveryChallanNums, appendFormData, mapToValueLabelArray } from "@prime-fresh/shared/modules";
...
export const MultipleCashVoucherForm = () => {
  ...
  const onFormPreview = (values: Omit<IMultiCashVoucher,'id'>) => {
    const data: any = {
      ...values,
      grnNo: values.grnNo ? allGRNNums.find(nums => nums.value === data.grnNo)?.label : null,
      challanNo: values.challanNo ? allDCNums.find(nums => nums.value === data.challanNo)?.label : null,
      companyName: values.companyName ? companyNames.find(names => names.value === data.companyName)?.label : null,
    };
    ...
    <Box flex={1}>
      {isLoading ?
        ...
        (<FormikProvider key={voucherId === '' ? 'create-voucher' : 'update-voucher'} value={formik}>
          <form key={voucherId === '' ? 'create-form' : 'update-form'} onSubmit={formik.handleSubmit} encType="multipart/form-data">
            <Grid2 container columnSpacing={1} rowSpacing={1} padding={1}>
                        ...
                        >
                          ...
                          <Grid2 size={{ xs: 12, md: 1 }} sx={{ display: "flex", alignItems: "center", justifyContent: "space-around" }}>
                            {formik.values.particulars.length > 1 &&
                              <IconButton color="error" size="large" sx={{ marginTop: 2 }} onClick={() => handleRemove(index, formik)}>
                                <Close />
                              </IconButton>}
                            <IconButton color="primary" size="large" sx={{ marginTop: 2 }} onClick={() => handlePush(particularsInitialValue, formik)}>
                              <Add />
                            </IconButton>
                          </Grid2>
                        </Grid2>
              ...
              <Grid2 size={{ xs: 12, md: 2 }}>
                <TextInput
                  ...
                  name="totalAmt"
                  label="Total Amount"
                  value={formik.values.totalAmt || null}
                />
              </Grid2>
              <Grid2 size={{ xs: 12, md: 6 }}>
                <TextInput
