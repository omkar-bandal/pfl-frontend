...
import { transportPaymentVoucherInitialValue, PURCHASE_ARRAYS, PURCHASE_ROUTES, setTPVoucherFormPreview, transportPaymentVoucherSchema, useCreateTransportPaymentVoucher, useGetTransportPaymentVoucherForUpdateById, useUpdateTransportPaymentVoucherById } from "@prime-fresh/purchase/modules";
import { FormButtonGroup, ImageUploadMultiple, PageTitle, RadioGroupInput, SelectInput, TextInput, toast } from "@prime-fresh/ui_shared";
...
import { useGetCompanyNames, useGetAllGRNNums, mapToValueLabelArray, appendFormData, numToWords } from "@prime-fresh/shared/modules";
...
export const TransportPaymentVoucherForm = () => {
  ...
  const onFormPreview = (values: ITranportPaymentVoucher) => {
    const data: any = {
      ...values,
  ...
  const { mutateAsync: mutatePatch, error: errorPatch, data: resPatch } = useUpdateTransportPaymentVoucherById(tpVoucherId);
    ...
    <Box flex={1}>
      {isLoading ?
        ...
        (<FormikProvider key={tpVoucherId === '' ? 'create-voucher' : 'update-voucher'} value={formik}>
          <form key={tpVoucherId === '' ? 'create-form' : 'update-form'} onSubmit={formik.handleSubmit} encType="multipart/form-data">
