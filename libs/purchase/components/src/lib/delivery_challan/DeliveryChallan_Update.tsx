...
export const DeliveryChallanUpdate = () => {
    ...
    const { data: dcData } = useGetDeliveryChallan(PURCHASE_API_URL.GET_A_DELIVERY_CHALLAN, dcId);
    const dcValues = dcData ? dcData : initValDeliveryChallan;

    //React Query to update delivery challan
    const { mutateAsync: mutatePatch } = useUpdateDeliveryChallan(PURCHASE_API_URL.UPDATE_DELIVERY_CHALLAN, dcId);
