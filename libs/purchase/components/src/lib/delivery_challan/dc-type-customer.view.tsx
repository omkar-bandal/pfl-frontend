...
type DCtypeCustomerViewPropType = {
...
export const DCtypeCustomerView: FC<DCtypeCustomerViewPropType> = (props) => {
  ...
  return (
    <div className={styles['document-container']} ref={contentRef}>
      ...
      <div className={styles['details-grid']}>
        {[
          { label: 'DATE:', value: dcData?.createdDate },
          { label: 'DC No:', value: dcData?.challanNo },
          { label: 'P.O. No:', value: dcData?.poNumber },
          { label: 'Vendor Code:', value: 'ABC Private Limited' },
          { label: 'Number of Crates:', value: '70' },
          { label: 'Grand Total:', value: dcData?.netProductWeight },
          { label: 'Dispatch Time:', value: '10:15 AM' },
          { label: 'Dispatch Vehicle No:', value: dcData?.vehicleNo },
          { label: 'Driver Name:', value: dcData?.driverName },
          { label: 'Driver Mobile No:', value: dcData?.contactNo },
          ...
          <div key={index} className={`${styles['details-item']} ${index === 12 ? styles['span-2'] : ''}`}>
            <span className={`${styles['text-smr']} ${styles['text-bold']} ${styles['label-mr']}`}>{item.label}</span>
      ...
      <div className={styles['address-challan-container']}>
        <div className={styles['address-box']}>
          <span className={`${styles['text-smr']} ${styles['text-bold']} ${styles['label-mb']}`}>Billing Address:</span>
          <span className={styles['text-smr']}>
            {formatAddress(customerData?.data?.deliveryDetails.deliveryAddress)}
          </span>
        </div>
        <div className={styles['address-box']}>
          ...
          <span className={styles['text-smr']}>
            {formatAddress(customerData?.data?.billingDetails.billingAddress)}
          </span>
        </div>
      ...
      <table className={styles['product-table']}>
        ...
        <tbody>
          {dcData?.deliveryChallanProducts.map((product, index) => (
            <tr>
            <th className={styles['sr-col']}>{index}</th>
            <th className={styles['product-col']}>{product.productName.productName || ''}</th>
            <th className={styles['uom-col']}>{product.saleUoM.unit || ''}</th>
