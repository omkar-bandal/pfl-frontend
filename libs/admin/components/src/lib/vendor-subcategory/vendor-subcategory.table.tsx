...
import { useGetAllVendorSubcategories } from '@prime-fresh/admin/modules';
...
export function VendorSubcatTable() {
  ...
  const vendorSubcategoryColumns = useVendorSubcategoryColumns(navigate);

  const tableNavActionConfig: TableNavActionsConfig = {
    tableId: adminTableIds.VENDOR_SUBCAT_TABLE_ID,
    createPath: adminRoutes.CREATE_VENDORS_SUBCAT,
    editPath: adminRoutes.UPDATE_VENDORS_SUBCAT,
  };
  ...
  const tableConfig = useDataTableFunctions({
    columnDef: vendorSubcategoryColumns,
    initialPageSize: 10,
    tableId: adminTableIds.VENDOR_SUBCAT_TABLE_ID,
  });
  ...
  const { data, isLoading, isError, error } = useGetAllVendorSubcategories(tableConfig.queryParams, debouncedSearch);
  ...
  return (
    <Box sx={{ flex: 1 }}>
      <TableHeader
        key={adminTableIds.VENDOR_SUBCAT_TABLE_ID}
        isMobile={isMobile}
        pageTitle="Vendor Subcategories"
      ...
      />
      <DialogContainer
        dialogKey={adminTableIds.VENDOR_SUBCAT_TABLE_ID}
        dialogTitle="Delete Vendor Subcategory"
