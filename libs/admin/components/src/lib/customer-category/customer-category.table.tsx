...
export function CustomerCategoryTable() {
  ...
  const { data, isLoading, isError, error } = useGetAllCustomerCategories(queryParams, debouncedSearch);
  ...
  const handleCreate = useCallback(() => navigate(ADMIN_ROUTES.CREATE_CUSTOMER_CAT), [navigate]);
  ...
  const handleEdit = useCallback(() => {
    ...
    if (selectedRows.length === 0) {
      ...
      navigate(`${ADMIN_ROUTES.UPDATE_CUSTOMER_CAT}/${selectedId}`);
    }
  }, [navigate, apiRef]);
