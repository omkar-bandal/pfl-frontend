...
export const useEmployeeColumns = (navigate: NavigateFunction): CustomGridColDef[] => {
  return useMemo(
    () => [
      {
        field: 'employeeId',
      ...
      },
      {
        ...
        minWidth: 180,
        hide: true,
        renderCell: (params: GridRenderCellParams<IEmployee>) => (params.row.accessLocation || []).join(', '),
      },
      {
        ...
        renderCell: (params: GridRenderCellParams) => {
          switch (params.row.status) {
            case 'ACTIVE':
              return (
                <Chip
                  label={convertInTitleCase(params.row.status)}
                  color="success"
                  size="small"
      ...
      },
      {
        ...
        renderCell: (params: GridRenderCellParams) => (
          <IconButton aria-label="edit" onClick={() => navigate(`${adminRoutes.UPDATE_EMPLOYEE}/${params.row.id}`)}>
            <Edit color="info" />
          </IconButton>
        ),
      },
      {
        ...
        disableColumnMenu: true,
        renderCell: (params: GridRenderCellParams) => (
          <IconButton aria-label="view" onClick={() => navigate(`${adminRoutes.VIEW_EMPLOYEE}/${params.row.id}`)}>
