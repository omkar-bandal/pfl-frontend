...
import { Grid2, InputAdornment, SelectChangeEvent, Typography } from '@mui/material';
import {
  returnedByCustomerInitialValues,
  returnedByCustomerSchema,
  salesRoutes,
  useCreateReturnedByCustomer,
} from '@prime-fresh/sales/modules';
...
export const ReturnedByCustomerForm = () => {
  ...
  const formik = useFormik({
    ...
    initialValues: returnedByCustomerInitialValues,
  ...
  const handleDcNumChange = useCallback(
    (dcId: string, values: PostReturnByCustomer, setFieldValue: any) => {
  ...
  const handleSubmit = (values: any) => {
    mutateAsyncPost(values)
      .then(() => {
        ...
        setTimeout(() => {
          navigate(salesRoutes.GET_ALL_RBC);
        }, 2000);
      })
      .catch(() => {
