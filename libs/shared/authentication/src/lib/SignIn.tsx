import { Grid, TextField, Typography, Button, OutlinedInput, InputAdornment, IconButton, Container, Box, CircularProgress } from '@mui/material';
import { useFormik } from 'formik';
import { useNavigate } from 'react-router-dom';
import { AUTH_ROUTES, STRINGS } from './constants';
import React, { useState } from 'react';
import { SignInWithEmailSchema, SignInWithMobileNoSchema } from './validation_schema';
          ...
          <Grid item sm={6} xs={12}>
            <form onSubmit={formik.handleSubmit}>
              <Grid container direction="column" spacing={0.5} padding={1}>
                <Grid item xs={12}>
                  <Typography variant='h5' component="div" textAlign="center" sx={{ fontWeight: 600, marginBottom: 2 }}>Sign In OLD</Typography>
                </Grid>
                <Grid item xs={12}>
                ...
                <Grid item xs={12}> 
                  <Button
                    ...
                    disableElevation
                    disabled={(!formik.values.uid && !formik.values.password) || isPending || isSuccess}
                  >
                    {isPending ? <CircularProgress size={25} color="inherit" /> : STRINGS.SIGN_IN}
                  </Button>
                </Grid>
                <Grid item xs={12}>
                  {isError && (
