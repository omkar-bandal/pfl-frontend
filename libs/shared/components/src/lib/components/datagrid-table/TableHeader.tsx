...
import { TableSearchBox, ActionMenu } from './components';
...
export const TableHeader: FC<TableHeaderProps> = memo(
  ({
    ...
    return (
      <Grid2 container spacing={2} marginY={1} alignItems="center">
        <Grid2 size={{ xs: isMobile ? 8 : 12, md: 12 }}>
        ...
        <Grid2 size={{ xs: 12, md: 5 }}>
          <TableSearchBox name="search" value={searchText} onChange={(e) => setSearchText(e.target.value)} />
        </Grid2>
        {!isMobile && (
          <Grid2
            size={{ xs: 12, md: 7 }}
            sx={{ display: 'flex', justifyContent: isMobile ? 'center' : 'flex-end', alignItems: 'center' }}
          >
