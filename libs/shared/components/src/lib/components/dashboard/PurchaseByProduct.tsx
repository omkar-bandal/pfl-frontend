...
import { useGetProductsPartialData, useGetPurchaseDataByProduct } from '@prime-fresh/shared/modules';
import { PageTitle, LineChart, DateInput, NonFormikAutocomplete } from '@prime-fresh/ui_shared';
import { nextPage, prevPage, queryParamsState, useAppDispatch, useAppSelector } from '@prime-fresh/modules';
...
export const PurchaseByProduct = () => {
  ...
  const { data: products, isLoading, isFetching } = useGetProductsPartialData(queryParams);
    ...
    <Box flex={1} margin={2}>
      <Grid2 container spacing={1} marginY={1}>
        ...
        <Grid2 size={{ xs: 12, md: 6 }} sx={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center' }}>
          <NonFormikAutocomplete<ProductPartialData, 'id'>
            name="productId"
            label="Product"
            value={productId}
            isLoading={isLoading}
            ...
            // hasNextPage={queryParams.page < (products?.totalPages || 1)}
