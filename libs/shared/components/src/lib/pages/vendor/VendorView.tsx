...
import {
  convertInTitleCase,
  formatAddress,
  useChangeStatusVendor,
  useGetVendorByIdForView,
} from '@prime-fresh/shared/modules';

export const VendorView = () => {
  const { id } = useParams<{ id: string }>();
  const { data: Vendor, isLoading } = useGetVendorByIdForView(id as string);
  const vendor = Vendor?.data ? Vendor?.data : null;
  const contentRef = useRef<HTMLDivElement>(null);
  const reactToPrintFn = useReactToPrint({ contentRef });
  console.log(Vendor);

  const { mutateAsync, error, data, isPending, isError } = useChangeStatusVendor(id as string);

  const changeVendorStatus = (status: StatusType) => {
    mutateAsync(status)
      .then(() => {
        toast.success(data?.message ? data.message : `Vendor registration ${status} successfully.`);
      })
      .catch(() =>
        toast.error(error?.message ? error?.message : 'Error while changing status of vendor registration.')
      );
  };
          ...
          <Grid container rowSpacing={1} marginY={2}>
            ...
            <Grid item xs={12} md={8} sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
              <BtnSmall
                ...
                onClick={() => changeVendorStatus('approved')}
              />
              <BtnSmall
                label="Disapprove"
                ...
                color="error"
                disabled={isPending && !isError}
                onClick={() => changeVendorStatus('notApproved')}
              />
                    ...
                    <div className={styles.gridContainer} style={{ padding: `0px 10px` }}>
                      ...
                      <div className={`${styles.gridItem} ${styles.span_9}`}>
                        <span className={`${styles.textSM} ${styles.mr}`}>
                          {convertInTitleCase(vendor?.companyName || '')}
                        </span>
                      </div>
                      <div className={`${styles.gridItem} ${styles.span_3}`}>
                      ...
                      <div className={`${styles.gridItem} ${styles.span_3}`}>
                        <span className={`${styles.textSM} ${styles.mr}`}>
                          {formatDate(String(vendor?.dateOfIncorporation) || '')}
                        </span>
                      </div>
                      <div className={`${styles.gridItem} ${styles.span_3}`}>
                    ...
                    <div className={styles.gridContainer} style={{ padding: `0px 10px` }}>
                      ...
                      <div className={`${styles.gridItem} ${styles.span_9}`}>
                        <span className={`${styles.textSM} ${styles.mr}`}>{`${
                          ...
                          vendor?.vendorSaleInfo?.contactLName || ''
                        }`}</span>
                      </div>
                      <div className={`${styles.gridItem} ${styles.span_3}`}>
                        <span className={`${styles.textSM} ${styles.textBold} ${styles.mr}`}> Contact No </span>
                      ...
                      <div className={`${styles.gridItem} ${styles.span_9}`}>
                        <span className={`${styles.textSM} ${styles.mr}`}>{`${
                          vendor?.vendorSaleInfo?.directContactNumber || ''
                        }, ${vendor?.vendorSaleInfo?.mobileNumber || ''}`}</span>
                      </div>
                      <div className={`${styles.gridItem} ${styles.span_3}`}>
                        <span className={`${styles.textSM} ${styles.textBold} ${styles.mr}`}> Email </span>
                      </div>
                ...
                <section>
                  <div className={styles.blockSection}>
                    <div className={styles.gridContainer} style={{ padding: `0px 10px` }}>
                      ...
                      <div className={`${styles.gridItem} ${styles.span_9}`}>
                        <span className={`${styles.textSM} ${styles.mr}`}>{`${
                          vendor?.vendorBankDetails?.beneficiaryFName || ''
                        } ${vendor?.vendorBankDetails?.beneficiaryMName || ''} ${
                          vendor?.vendorBankDetails?.beneficiaryLName || ''
                        }`}</span>
                      </div>
                      <div className={`${styles.gridItem} ${styles.span_3}`}>
                      ...
                      <div className={`${styles.gridItem} ${styles.span_9}`}>
                        <span className={`${styles.textSM} ${styles.mr}`}>
                          {formatAddress(vendor?.vendorBankDetails.branchAddress)}
                        </span>
                      </div>
                      <div className={`${styles.gridItem} ${styles.span_3}`}>
                      ...
                      <div className={`${styles.gridItem} ${styles.span_3}`}>
                        <span className={`${styles.textSM} ${styles.mr}`}>
                          {vendor?.vendorBankDetails.invoiceCurrency}
                        </span>
                      </div>
                      <div className={`${styles.gridItem} ${styles.span_6}`}>
                        <span className={`${styles.textSM} ${styles.textBold} ${styles.mr}`}>
                          Is Copy of Cancelled Cheque Attached ?{' '}
                        </span>
                      </div>
                      <div className={`${styles.gridItem} ${styles.span_6}`}>
                        <span className={`${styles.textSM} ${styles.mr}`}>
                          {vendor?.vendorBankDetails.cancelledChequeCopy ? `Yes` : `No`}
                        </span>
                      </div>
                    </div>
                  </div>
