...
import { DashboardDataInitValue } from '@prime-fresh/services';
import {
...
import { Select, DashboardCard, PurchaseByFarmer, PurchaseByProduct, PurchaseByVendor } from '../../components';
...
export const Dashboard = () => {
  ...
  const dashboardCardData = [
    ...
    {
      label: 'Purchase From Farmer',
      color: '#ff9900',
      quantityCount: totalPurchaseByFarmer.totalQuantityInKg || 0,
      quantityPercent: Number(
        (((totalPurchaseByFarmer.totalQuantityInKg || 1) / (totalPurchase.totalQuantityInKg || 1)) * 100).toFixed(2)
      ),

      amountCount: totalPurchaseByFarmer.totalAmount || 0,
      amountPercent: Number(
        (((totalPurchaseByFarmer.totalAmount || 1) / (totalPurchase.totalAmount || 1)) * 100).toFixed(2)
      ),
    ...
    },
    {
      ...
      quantityPercent: Number(
        ((totalpurchaseByVendor.totalQuantityInKg || 1) / (totalPurchase.totalQuantityInKg || 1)) * 100
      ).toFixed(2),
      amountCount: totalpurchaseByVendor.totalAmount || 0,
      amountPercent: Number(
        ((totalpurchaseByVendor.totalAmount || 1) / (totalPurchase.totalAmount || 1)) * 100
      ).toFixed(2),
    ...
    },
    {
      ...
      quantityPercent: Number(((dump.totalQuantity || 1) / (totalPurchase.totalQuantityInKg || 1)) * 100).toFixed(2),
      amountCount: dump.totalAmount || 0,
      amountPercent: Number(((dump.totalAmount || 1) / (totalPurchase.totalAmount || 1)) * 100).toFixed(2),
      onClickAction: () => setSelectedCard('Total Dump'),
    },
    {
      label: 'Total Rejection',
      quantityCount: rejection.totalQuantity || 0,
      quantityPercent: Number(((rejection.totalQuantity || 1) / (totalPurchase.totalQuantityInKg || 1)) * 100).toFixed(
        2
      ),
      amountCount: rejection.totalAmount || 0,
      amountPercent: Number(((rejection.totalAmount || 1) / (totalPurchase.totalAmount || 1)) * 100).toFixed(2),
      color: '#ff3333',
      ...
      <Grid2 container spacing={2}>
        <Grid2 size={{ xs: 12, md: 4 }}>
          <Select
            ...
            options={sharedData.dashboardPeriodFilters}
