...
import { FarmDetails, FarmerDetails, CropDetails, FarmerImageSection, FarmerFormPreviewConfig } from './form-sections';
...
export const FarmerForm = () => {
  ...
  const { data, isLoading } = useGetFarmerByIdForUpdate(farmerId);
  const farmerData = data?.data ? data.data : initValFarmer;
  const farmerInitVal = farmerId === '' ? initValFarmer : farmerData;
  
  const farmerFormSteps = ['Farmer Details', 'Farm Details', 'Crop Details', 'Images', 'Preview'];
  ...
  console.log('Farmer: ', formik.values);
    ...
    farmerId === ''
      ? mutatePost(formData)
          .then(() => {
            ...
            setTimeout(() => {
              navigate(sharedRoutes.VIEW_ALL_FARMER);
            }, 2000);
          })
          .catch(() => {
  ...
  const farmerFormPreviewData = {
    ...formik.values,
    crops: formik.values.crops.map((prod) => ({
      ...prod,
      crop: products?.find((item) => item.id === prod.crop)?.name,
    })),
  };
  return isLoading ? (
    <Box flex={1}>
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      >
        <Grid2 container>
          ...
          <Grid2 size={12}>{!isMobile && <FormStepper activeStep={activeStep} steps={farmerFormSteps} />}</Grid2>
        </Grid2>
      </Box>
      <FormScrollContainer>
        <FormikProvider value={formik}>
          <form onSubmit={formik.handleSubmit} encType="multipart/form-data">
            <Grid2 container columnSpacing={1} rowSpacing={1} padding={1}>
              ...
              {activeStep === 2 && <CropDetails products={products} />}
              {activeStep === 3 && <FarmerImageSection />}
              {activeStep === 4 && <DataViewer config={FarmerFormPreviewConfig} data={farmerFormPreviewData} />}
            </Grid2>
          </form>
        </FormikProvider>
      </FormScrollContainer>
