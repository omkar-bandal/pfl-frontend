...
export class PackagingMaterialService extends BaseService {
  ...
  getAllPackagingMaterials(
    ...
    const url = adminApiUrlConstants.GET_ALL_PACKAGING_MATERIAL(queryParams);
    console.log(url);
    return this.get(url);
  }

  getPackagingMaterialById(
    id: string
  ): Promise<ApiBaseState<GetPackagingMaterial>> {
    const url = `${adminApiUrlConstants.GET_A_PACKAGING_MATERIAL}/${id}`;
    return this.get(url);
  }

  updatePackagingMaterial(
    id: string,
    data: GetPackagingMaterial
  ): Promise<ResultModel> {
    const url = `${adminApiUrlConstants.UPDATE_PACKAGING_MATERIAL}/${id}`;
    return this.patch(url, data);
  }

  deletePackagingMaterialById(id: string): Promise<ResultModel> {
    const url = `${adminApiUrlConstants.DELETE_PACKAGING_MATERIAL}/${id}`;
