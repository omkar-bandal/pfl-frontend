...
export class AQRServices extends BaseService {
  ...
  getAllAQRs(queryParams?: QueryParams): Promise<ApiBaseState<GetAQR[]>> {
    const url = inventoryApiUrl.GET_ALL_AQR(queryParams);
    return this.get(url);
  }

  getAQRForUpdateById(id: string): Promise<ApiBaseState<GetAQR>> {
    const url = `${inventoryApiUrl.GET_AQR_FOR_UPDATE_BY_ID}/${id}`;
    return this.get(url);
  }

  getAQRForViewById(id: string): Promise<ApiBaseState<GetAQR>> {
